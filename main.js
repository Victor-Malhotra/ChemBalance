...
document.querySelector("#plus-button").addEventListener("click", function () {
  ...
  document
    .querySelector("#changeMoleculeMass")
    .addEventListener("keyup", function (event) {
      document.querySelector("#changeMoleculeMoles").value = (
        ((document.querySelector("#changeMoleculeMass").value / molarMass) *
          1000) /
        document.querySelector("#changeMoleculeAmount").value
      ).toFixed(3);
      savedMoles = document.querySelector("#changeMoleculeMoles").value;

      updateMoleculeData(
        document.querySelector("#changeMoleculeMass").value,
        document.querySelector("#changeMoleculeMoles").value,
        document.querySelector("#changeMoleculeAmount").value
      );
    });
  document
    .querySelector("#changeMoleculeMoles")
    .addEventListener("keyup", function (event) {
      savedMoles = document.querySelector("#changeMoleculeMoles").value;
      document.querySelector("#changeMoleculeMass").value = (
        (document.querySelector("#changeMoleculeMoles").value * molarMass) /
        1000
      ).toFixed(3);
  ...
  document
    .querySelector("#changeMoleculeAmount")
    .addEventListener("keyup", function (event) {
      document.querySelector("#changeMoleculeMoles").value =
        savedMoles / document.querySelector("#changeMoleculeAmount").value;
  ...
  document
    .querySelector("#changeMoleculeData")
...
function colorElement(element) {
  ...
  if (
    element.group - element.period == 11 ||
...
function displayAtom(element) {
  ...
  document
    .querySelector("#addAtomMass")
    .addEventListener("keyup", function (event) {
      document.querySelector("#addAtomMoles").value = (
        ((document.querySelector("#addAtomMass").value /
          parseFloat(currentElement.atomicMass.slice(0, -3))) *
          1000) /
        document.querySelector("#addAtomAmount").value
      ).toFixed(5);
      savedMoles = document.querySelector("#addAtomMoles").value;
    });
  document
    .querySelector("#addAtomMoles")
    .addEventListener("keyup", function (event) {
      savedMoles = document.querySelector("#addAtomMoles").value;
      document.querySelector("#addAtomMass").value = (
        (document.querySelector("#addAtomMoles").value *
  ...
  document
    .querySelector("#addAtomAmount")
    .addEventListener("keyup", function (event) {
      document.querySelector("#addAtomMoles").value =
        savedMoles / document.querySelector("#addAtomAmount").value;
    });

  document
    .querySelector("#addAtom")
    .addEventListener("submit", function (event) {
      event.preventDefault();
      let atomHTML = ``;
      if (document.querySelector("#addAtomAmount").value != 1) {
        atomHTML = `
            <p id="atomic_symbol">${element.symbol}<sub>${
          document.querySelector("#addAtomAmount").value
        } </sub> </p>
            `;
      } else {
...
function updateChemicalFormula(atom) {
  ...
  atom["moles"] =
    document.querySelector("#addAtomMoles").value / parseFloat(atom["amount"]);
...
function solve(x) {
  ...
  for (pivot = 0; pivot < Math.min(molecules, elems.size); pivot++) {
    ...
    for (i = 0; i < workingOnThisRow.length; i++)
      workingOnThisRow[i] *= multiplyWhat;
  ...
  if (rrefArray[0][elems.size] == 0 || rrefArray[0][elems.size] == undefined)
    return "Nope!";
